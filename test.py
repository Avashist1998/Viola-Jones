...
A.to_csv('/Users/abhay/Documents/GitHub/Viola-Jones_Algorithm/10_'+str(gama)+'_round_results.csv', index=None,float_format= '%10.5f')
gama = 0.8
...
sum_error = totoal_error(test_X,test_y,theta,parity_tol)
A = pd.DataFrame({'round':np.arange(1, rounds+1, 1),'beta':beta_list,'J_values':j_of_round,'theat':theta,'emprical':e_t[:,0],'False Negative':e_t[:,1],'False Positive':e_t[:,2],'pairty':parity_tol,'emprical_total':sum_error[:,0],'False Negative_total':sum_error[:,1],'False Positive_total':sum_error[:,2]})
A.to_csv('/Users/abhay/Documents/GitHub/Viola-Jones_Algorithm/10_'+str(gama)+'round_results.csv', index=None,float_format= '%10.5f')
print('complete')