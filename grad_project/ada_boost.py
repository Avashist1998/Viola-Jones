...
def decision_stamp(S,Distribution):
    ...
    processed_list = Parallel(n_jobs=num_cores,prefer = 'threads')(delayed(parallel_sort)(S_np,j,row) for j in range(col-2))
...
def ada_boost(S,y,rounds):
    ...
    for i in range(0,rounds):
        ...
        [e1, e2,e3] = error_calculator(prediction,y)
        if e1 <= 0.5:
            parity = 1
        else :
            parity = -1
...
def total_error(test_X,test_y,theta,parity_tol):
    ...
    threshold_total = (np.multiply(parity_tol,test_np_X[:,j_of_round[:]]) < np.multiply(1,theta)).astype(int)
    predictions_total = []
    combined_error = []
    alpha = np.log(1/np.array(beta_list))
    for i in range(1,len(alpha)+1):
        prediction_t = (np.dot(threshold_total[:,:i],alpha[:i]) >= 0.5*sum(alpha[:i])).astype(int)
        predictions_total.append(prediction_t)
    predictions_total = np.array(predictions_total)
    sum_error = []
    for i in range(len(alpha)):
        sum_error.append(error_calculator(2*predictions_total[i]-1,test_np_y))
